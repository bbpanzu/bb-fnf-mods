...
class PlayState extends MusicBeatState
{
	...
	public var BF_X:Float = 0;
	public var BF_Y:Float = 0;
	public var DAD_X:Float = 0;
	public var DAD_Y:Float = 0;
	public var GF_X:Float = 0;
	public var GF_Y:Float = 0;
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case "cabinet-battle-i"|"cabinet-battle-ii":
				defaultCamZoom = 1;
				curStage = 'hamilton';
				bgg = new BGSprite('hamilton/bg', -225.4, -204.9, 0, 0);
				var white:FlxSprite = new FlxSprite(0, 0).makeGraphic(1280, 720);
				white.scrollFactor.set();
				add(white);
				
				endCutscene = new FlxSprite(0, 0);
				endCutscene.scrollFactor.set();
				endCutscene.frames = Paths.getSparrowAtlas("hamilton/endcutscene");
				endCutscene.animation.addByPrefix("end", "end", 8, false);
				endCutscene.animation.stop();
				endCutscene.alpha = 0;
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String, ?onLua:Bool = false) {
		switch(eventName) {
			...
			case 'BG Color':
				if(curStage == 'hamilton' && !ClientPrefs.lowQuality) {
					bgg.color = Std.parseInt(value1);
				}
	...
	function endSong():Void
	{
		...
		if (isStoryMode)
		{
			...
			if (storyPlaylist.length <= 0)
			{
				...
				endCutscene.animation.play("end", true,false,3);
				FlxTween.tween(endCutscene, {alpha:1}, 0.3);
				FlxTween.tween(camHUD, {alpha:0}, 0.3);
				FlxG.sound.play(Paths.sound("endcutscene")).onComplete = leave;
